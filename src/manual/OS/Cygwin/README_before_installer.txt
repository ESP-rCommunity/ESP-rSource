...
Have a look at the Cygwin web site at <http://www.cygwin.com>. It contains lots of useful information in addition to a small setup.exe application!
 
Prior to downloading and installing cygwin ensure there is an administrative and at least one normal user on your PC. Typically the Administrator user would maintains the cygwin and ESP-r applications. If you work as a different user for normal simulation tasks then it is harder to corrupt databases and test models.
 
ESP-r works much better if user names do not have spaces or unprintable characters. ESP-r also works better if you setup your user account to have a home folder like C:\Joe rather than the typical C:\Documents and Settings\Joe. 
...
1.4 Post-install patches: After completing the list the cygwin installer will download the various packages and put an icon on the desktop. The installer takes some time sorting out the post-install so do not give up!
...
The bin folder is populated with the graphic version of ESP-r. If you want to run in text mode you need to start the modules with a -mode text in the command line.
...
Setup does not change the system 'path' environment variables. You will need to set an environment variable PATH so that the folder /usr/esru/esp-r/bin is known. (In the Cygwin command window) Go back to your home folder and see if there is a .cshrc or a .profile file.
