...
updview_(ifsc,itfsc,vl,vr,vt,vb,g3w,g3h,lttyc)
long int ifsc,itfsc,vl,vr,vt,vb,g3w,g3h,lttyc;
{
 ...
 */
  ...
  do {	/* Read a line from file, skipping & stripping comments */
    ...
    } else if (strncmp(word, "*edwline", 8) == 0) {
        iexp = 4;
        cstripc(wwc,outstr,&iexp,&itm,"edwline cmd",&ier);
        sscanf(outstr,"%d%d%d%d",&x1,&y1,&x2,&y2);
        edwline_(&x1,&y1,&x2,&y2);
    } else if (strncmp(word, "*eswline", 8) == 0) {
        iexp = 4;
        cstripc(wwc,outstr,&iexp,&itm,"eswline cmd",&ier);
        sscanf(outstr,"%d%d%d%d",&x1,&y1,&x2,&y2);
        eswline_(&x1,&y1,&x2,&y2);
    } else if (strncmp(word, "*edash", 6) == 0) {
        iexp = 5;
        cstripc(wwc,outstr,&iexp,&itm,"edash cmd",&ier);
        sscanf(outstr,"%d%d%d%d%d",&x1,&y1,&x2,&y2,&ipdis);
        edash_(&x1,&y1,&x2,&y2,&ipdis);
    } else if (strncmp(word, "*echain", 7) == 0) {
        iexp = 5;
        cstripc(wwc,outstr,&iexp,&itm,"edash cmd",&ier);
        sscanf(outstr,"%d%d%d%d%d",&x1,&y1,&x2,&y2,&ipdis);
    ...
    } else if (strncmp(word, "*winfnt", 7) == 0) {
        iexp = 1;
        cstripc(wwc,outstr,&iexp,&itm,"winfnt cmd",&ier);
        sscanf(outstr,"%d",&size);
        winfnt_(&size);
    ...
    } else if (strncmp(word, "*wstxpt", 7) == 0) {
        iexp = 0;
        cstripc(wwc,outstr,&iexp,&itm,"wstxpt cmd",&ier);
        sscanf(outstr,"%d%d",&x,&y);
        cstripc(wwc,outstr,&iexp,&itm,"wstxpt msg",&ier);
        wstxpt_(&x,&y,outstr,strlen(outstr));
