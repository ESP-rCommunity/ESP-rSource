...
while ( my ($key, $revision ) = each  %revisions ) {
  ...
  # Progress update 
  stream_out("\nChecking out $revision from ESP-r central...");
...
while ( my ($key, $revision ) = each  %revisions ) {
  ...
  # Progress update 
  stream_out("\nBuilding $key ($revision) version of ESP-r for use with Forcheck.");
...
# Loop trough revisions...
while ( my ($key, $revision ) = each  %revisions ) {

  # progress update
  stream_out("\nRunning Forcheck static analyzer on $key version.");
...
foreach my $bin ( sort keys %binlist ){
  ...
  # Summarize results
  $results .= "   Total errors:     $revisions{\"reference\"} $old_msgs{\"Total error messages\"}, $revisions{\"test\"} $new_msgs{\"Total error messages\"} \n";
  $results .= "   Total warnings:   $revisions{\"reference\"} $old_msgs{\"Total warning messages\"}, $revisions{\"test\"} $new_msgs{\"Total warning messages\"} \n";
  $results .= "   Total info msgs:  $revisions{\"reference\"} $old_msgs{\"Total informational messages\"}, $revisions{\"test\"} $new_msgs{\"Total informational messages\"} \n";
  
  # Now append unmatched errors to result string 
  if ( $bin_fail ){
    $results .= "   Summary of differences:\n";
  }
  foreach my $code ( sort @error_codes ){
    $results .= "     -> $code [ Instances: $revisions{\"reference\"} $old_msgs{$code}, $revisions{\"test\"} $new_msgs{$code} ]\n";
  }
  foreach my $code ( sort @warning_codes ){
    $results .= "     -> $code [ Instances: $revisions{\"reference\"} $old_msgs{$code}, $revisions{\"test\"} $new_msgs{$code} ]\n";
  }
  foreach my $code ( sort @info_codes ){
    $results .= "     -> $code [ Instances: $revisions{\"reference\"} $old_msgs{$code}, $revisions{\"test\"} $new_msgs{$code} ]\n";
  }
...
$output .= "Regression test                      (see below)\n";
$output .= "------------------------------------------------\n";
